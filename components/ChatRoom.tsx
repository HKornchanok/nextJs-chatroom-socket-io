...
export default function ChatRoom() {
  const { socket, userType, userName, userId, setUserType, setUserName } =
  ...
  useEffect(() => {
    ...
    socket.on('userJoined', (data: { user: User; type: string }) => {
      ...
      const userWithDates = {
        ...
        lastActivity: data.user.lastActivity
          ? new Date(data.user.lastActivity)
          : undefined,
        sessionStartTime: data.user.sessionStartTime
          ? new Date(data.user.sessionStartTime)
          : undefined,
      };
    ...
    socket.on(
      'userLeft',
      (data: { userId: string; userType: string; userName?: string }) => {
        ...
        setPendingGuests(prev =>
          prev.filter(guest => guest.id !== data.userId)
        );
    ...
    socket.on('roomState', (data: { users: Users; messages: Message[] }) => {
      ...
      const usersWithDates = {
        admin: data.users.admin
          ? {
              ...data.users.admin,
              joinedAt: data.users.admin.joinedAt
                ? new Date(data.users.admin.joinedAt)
                : undefined,
              lastActivity: data.users.admin.lastActivity
                ? new Date(data.users.admin.lastActivity)
                : undefined,
              sessionStartTime: data.users.admin.sessionStartTime
                ? new Date(data.users.admin.sessionStartTime)
                : undefined,
            }
          : null,
        guest: data.users.guest
          ? {
              ...data.users.guest,
              joinedAt: data.users.guest.joinedAt
                ? new Date(data.users.guest.joinedAt)
                : undefined,
              lastActivity: data.users.guest.lastActivity
                ? new Date(data.users.guest.lastActivity)
                : undefined,
              sessionStartTime: data.users.guest.sessionStartTime
                ? new Date(data.users.guest.sessionStartTime)
                : undefined,
            }
          : null,
        pendingGuests: data.users.pendingGuests.map(guest => ({
          ...
          lastActivity: guest.lastActivity
            ...
            : undefined,
          sessionStartTime: guest.sessionStartTime
            ? new Date(guest.sessionStartTime)
            : undefined,
        })),
      };
        ...
        <div className="flex-1 flex flex-col w-full">
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-6">
            {messages.map(message => {
              const isOwnMessage =
                message.senderId === userId || message.user === userName;

              // Show system messages to admins, and certain system messages to guests
              if (message.type === 'system' && userType !== 'admin') {
                // Allow session-related system messages for guests
                if (
                  message.message.includes('session') ||
                  message.message.includes('expire') ||
                  message.message.includes('Warning') ||
                  message.message.includes('approved') ||
                  message.message.includes('rejected')
                ) {
                  // Continue to show these messages
                } else {
                  return null;
                }
              }
          ...
          <div className="bg-white border-t p-4 min-w-0">
            <form
              ...
              className="flex space-x-3 min-w-0"
            >
              <input
                type="text"
                value={newMessage}
                ...
                {users.guest && (
                  <div
                    className={`flex items-center justify-between p-3 rounded-lg ${
                      ...
                      currentTime.getTime() -
                        users.guest.sessionStartTime.getTime() >
                        240000
                        ? 'bg-red-50'
                        : users.guest.lastActivity &&
                            currentTime.getTime() -
                              users.guest.lastActivity.getTime() >
                              60000
                          ? 'bg-yellow-50'
                          : 'bg-green-50'
                    }`}
                    ...
                    >
                      ...
                      <div className="flex space-x-1">
                        <button
                          ...
                          title={
                            users.guest
                              ? 'Room is full. Kick current guest first.'
                              : 'Approve guest'
                          }
                        >
                          Approve
          ...
          <div className="bg-white rounded-lg p-6 max-w-md w-full mx-4 shadow-xl">
            <div className="text-center">
              ...
              <p className="text-sm text-gray-500 mb-6">
                The admin has removed you from the chat room. You will be
