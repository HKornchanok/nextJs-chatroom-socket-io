...
class ChatRoom {
  ...
  checkSessionTimeouts(): string[] {
    ...
    if (this.guest && this.guest.sessionStartTime) {
      ...
      if (sessionTime > 300000) {
        // 300 seconds (5 minutes) in milliseconds
  ...
  shouldWarnSessionExpiry(): boolean {
    if (this.guest && this.guest.sessionStartTime) {
      const sessionTime =
        new Date().getTime() - this.guest.sessionStartTime.getTime();
  ...
  if (!(res.socket as any).server.io) {
    ...
    const inactivityCheckInterval = setInterval(() => {
      ...
      inactiveGuests.forEach(guestId => {
        ...
        if (guest && guest.id === guestId) {
          ...
          if (success) {
            io.to(guest.socketId).emit('kicked', {
              reason: 'Inactive for 90 seconds',
            });
      ...
      if (
        users.guest &&
        chatRoom.shouldWarnSessionExpiry() &&
        !users.guest.sessionWarningSent
      ) {
    ...
    io.on('connection', socket => {
          ...
          if (data.type === 'admin') {
            ...
            if (result && result.success) {
              ...
              if (result.previousAdmin) {
                io.to(result.previousAdmin.socketId).emit('kicked', {
                  reason: 'Another admin has joined',
                });
                io.emit('userLeft', {
                  userId: result.previousAdmin.id,
                  userType: 'admin',
                });
                io.emit('newMessage', {
                  id: Date.now().toString(),
              ...
              io.emit('pendingGuestCount', {
                count: users.pendingGuests.length,
              });
            } else {
              socket.emit('joined', {
                success: false,
      ...
      socket.on('sendMessage', async (message: string) => {
        const users = chatRoom.getUsers();
        const user =
          users.admin?.id === socket.id
            ? users.admin
            : users.guest?.id === socket.id
              ? users.guest
              : null;
        ...
        if (user && chatRoom.isUserInRoom(socket.id)) {
          ...
          if (user.type === 'guest') {
            console.log(
              `Guest ${user.name} sent message: "${message}" - Generating AI response...`
            );
            try {
              ...
              const recentMessages = chatRoom
                ...
                .filter(
                  msg =>
                    msg.type === 'message' &&
                    (msg.user === user.name ||
                      msg.user === CHATGPT_CONFIG.assistantName)
                )
                .slice(-10)
                .map(msg => ({
                  role:
              ...
              if (response.ok) {
                ...
                const delay =
                  CHATGPT_CONFIG.minDelay +
                  Math.random() *
                    (CHATGPT_CONFIG.maxDelay - CHATGPT_CONFIG.minDelay);
          ...
          if (success) {
            ...
            if (guest) {
              ...
              io.to(guest.socketId).emit('newMessage', {
                ...
                user: 'System',
                message:
                  'Your session will expire in 5 minutes. Make the most of your time!',
                timestamp: new Date(),
                type: 'system',
              });
            }
            ...
            const updatedUsers = chatRoom.getUsers();
            io.emit('pendingGuestCount', {
              count: updatedUsers.pendingGuests.length,
            });
          } else {
            // Room is full, cannot approve guest
            const pendingGuest = users.pendingGuests.find(
              g => g.id === guestId
            );
            if (pendingGuest) {
              io.emit('newMessage', {
      ...
      socket.on('cancelRequest', () => {
        ...
        const cancelingGuest = users.pendingGuests.find(
          g => g.id === socket.id
        );
